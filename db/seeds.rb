...
puts 'Generating brazilian ETFs...'

brazilian_etf_list_csv = File.read(Rails.root.join('db', 'csv', 'brazilian_etfs.csv'))
parsed_brazilian_etf_list_csv = CSV.parse(brazilian_etf_list_csv, headers: true, encoding: 'utf-8')

parsed_brazilian_etf_list_csv.each do |row|
  code = row['Ticker']
